                        ...
                        <form action="{{ route('aminUser.update') }}" method="post">
                            ...
                            <div class="row">
                                <div class="col-8">
                                    ...
                                    <input type="text" name="" disabled id="username"
                                        class="form-control @error('username') is-invalid  @enderror"
                                        value="{{ old('username') ?? $data->username }}">
                                </div>
                                <div class="col-4">
                                    <label for="is_role" class="form-label">Give A Role :</label>
                                    <select name="is_role" id="is_role"
                                        class="form-control @error('is_role') is-invalid  @enderror">
                                        <option value="">Give A Role</option>
                                        @foreach ($role as $rol)
                                            <option @if ($data->is_role == $rol->id) selected @endif
                                                value="{{ $rol->id }}">{{ Str::upper($rol->role_name) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-12 my-2">
                                    ...
                                    <input type="email" name="" disabled id="email"
